...
// Function to create a fly with movement behavior
const createFly = () => {
  const fly = document.createElement('img');
  fly.src = 'fly.png'; // Ensure the file is in the same folder as the script
  fly.alt = 'Fly';
  fly.width = 50;
  ...
  // Append the new fly to the container
  flyContainer.appendChild(fly);

  // Set the behavior when the real cursor or a fake cursor "touches" the fly
...
const moveFly = (fly) => {
  const x = Math.random() * (fly.offsetWidth + 2000);
  const y = Math.random() * ( fly.offsetHeight + 2000);
  fly.style.left = `${x}px`;
  fly.style.top = `${y}px`;
}
...
// Add a new fake cursor on each click
document.addEventListener('click', () => {
  // Create a new fake cursor image
  const fakeCursor = document.createElement('img');
  fakeCursor.src = 'cursor-image.png'; // Replace with your cursor image path
  ...
  // Add event listeners to make each fake cursor move flies like the real cursor
  fakeCursor.addEventListener('mouseenter', (event) => {
    const flyElements = document.querySelectorAll('#flyContainer img');
    flyElements.forEach((fly) => {
      if (isHovering(fakeCursor, fly)) {
        moveFly(fly);
        rotateFly(fly);
      }
    });
  ...
  // Append the fake cursor to the document
  document.body.appendChild(fakeCursor);
  fakeCursors.push(fakeCursor);
});

// Track mouse movement to move fake cursors
document.addEventListener('mousemove', (event) => {
  fakeCursors.forEach((fakeCursor, index) => {
    setTimeout(() => {
      fakeCursor.style.left = `${event.pageX + index * 5}px`;
      fakeCursor.style.top = `${event.pageY + index * 5}px`;
    }, index * 50);
  });
});

// Spawn a new fly every second
setInterval(createFly, 5000);
...
// Helper function to check if fake cursor is hovering over a fly
const isHovering = (cursor, fly) => {
  const cursorRect = cursor.getBoundingClientRect();
  const flyRect = fly.getBoundingClientRect();
  return !(
    cursorRect.top > flyRect.bottom ||
    cursorRect.bottom < flyRect.top ||
    cursorRect.left > flyRect.right ||
    cursorRect.right < flyRect.left
  );
};
...
var settings = {
  ...
  trailLength: 100000000, // how long is the trail?
